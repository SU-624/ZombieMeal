...
namespace SelectedEffectOutline
{
    ...
    public class Outline : MonoBehaviour
    {
        ...
        [Range(1f, 25f)] public float m_OutlineWidth = 20f;
        ...
        void Update()
        {
            ...
            if (m_TriggerMethod == ETriggerMethod.MouseLeftPress)
            {
                bool on = m_IsMouseOn && Input.GetMouseButtonDown(0);
                if (on)
                    OutlineEnable();
                else
                    OutlineDisable();
        ...
        void OnMouseEnter()
        {
            if (GameManager.Instance.isPopUp == false && !DayAndNight.dayandNight.isNextDay 
                && !GameManager.Instance.isCapturing && !GameManager.Instance.isMakingCookie && InteractionEvent.Instance.isEvent == true) // 밤낮이 바뀌는 동안은 상호작용 가능한 건물에 외곽선이 안나오게 하기 위해 
            {
                m_IsMouseOn = true;
